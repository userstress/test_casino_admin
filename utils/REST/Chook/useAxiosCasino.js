...
import axiosInstance from "./CallCasino"; // axiosInstance를 임포트합니다.
  ...
  const sendRequest = (method, url, headers, body, onComplete) => {
    ...
    axiosInstance({
      ...
      .then((response) => {
        ...
        setError(null);
        if (onComplete) {
          onComplete(null, response.data); // if 문을 사용하여 콜백 호출
        }
      })
      .catch((err) => {
        setData(null);
