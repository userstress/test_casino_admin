...
export default function UsedMoneyLogList() {
  ...
  async function fetchDataForCategory(category, isSearch) {
    ...
    try {
      ...
      const updatedData = response.data.map((item) => ({
        ...
        usedSportsBalance: ["환전", "회수", "베팅차감", "수동차감"].includes(item.category)
          ? item.usedSportsBalance * -1
          : item.usedSportsBalance,
      }))
  ...
  async function checkUserName(curCategory) {
    ...
    if (curCategory === "전체") {
      data = await fetchDataForCategory(null, false)
    } else {
      data = await fetchDataForCategory(curCategory, true)
    }
      ...
      <CustomBar>
        ...
        <section className={styles.totalmoney}>
          <div style={{ height: "100%" }}>
            <label style={{ fontSize: "12px", marginRight: "10px" }} htmlFor="usedmoney">
              유저 사용내역
            </label>
            <input
              id="usedmoney"
              value={addCommasToNumber(allMoney.hit * -1)}
          ...
          <div style={{ height: "100%" }}>
            ...
            <input
              id="winmoney"
              value={addCommasToNumber(allMoney.used * -1)}
              style={{ width: "70%", height: "100%", backgroundColor: "#D9D9D9", textAlign: "right" }}
              readOnly
            />
          </div>
