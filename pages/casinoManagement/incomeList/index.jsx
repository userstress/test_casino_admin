...
export default function index() {
  ...
  const fetchData = async () => {
    try {
      const statusOptions = ["스포츠머니 -> 카지노머니로 전환", "카지노머니로 전환 -> 스포츠머니"]
      ...
      renderCell: (params) => {
        return (
          <button
            ...
            onClick={(event) => sendingNote(event, params)}
          >
            쪽지
          </button>
        )
      ...
      <CustomHeader
        text={
          <div style={{ width: "100%", display: "flex", justifyContent: "space-between" }}>
            <div>전체 충환전 목록 (전체:{apiData ? apiData.length : "0"})</div>
