...
export default function index() {
  ...
  const fetchData = async () => {
    try {
      const statusOptions = ["카지노머니 -> 스포츠머니로 전환"]
      ...
      const filteredArray = apiDataCombined.filter((item) => item.description === "카지노머니 -> 스포츠머니로 전환")
      ...
      <CustomHeader
        text={
          <div style={{ width: "100%", display: "flex", justifyContent: "space-between" }}>
            <div>카지노 → 스포츠 목록 (전체:{apiData ? apiData.length : "0"})</div>
